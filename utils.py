...
class Check_Collisions:
    ...
    def check_line_rect_collision(self, line, rect):
        ...
        point1 = self.check_line_line_collision(line, (rect.x, rect.y, rect.x, rect.y + rect.height))  # West
        point2 = self.check_line_line_collision(line, (
            rect.x, rect.y + rect.height, rect.x + rect.width, rect.y + rect.height))  # North
        point3 = self.check_line_line_collision(line, (
            rect.x + rect.width, rect.y + rect.height, rect.x + rect.width, rect.y))  # Est
        point4 = self.check_line_line_collision(line, (rect.x, rect.y, rect.x + rect.width, rect.y))  # South
    ...
    def check_line_room_collision(self, line, room):
        ...
        point4 = self.check_line_line_collision(line, (rect.x, rect.y, rect.x + rect.width, rect.y))  # South
        if point1 is not None:
            if not room.door.sprite.rect.collidepoint(point1):  # QUESTA QUI UNICA VARIAZIONE
                intersection_points.append(point1)
        if point2 is not None:
    ...
    def check_line_line_collision(self, line1, line2):
        x1 = line1[0]
        y1 = line1[1]
        x2 = line1[2]
        y2 = line1[3]
